...
"""Tasks with a laptop on the tabletop."""
...
laptop_uniform_position = distributions.Uniform(
    low=[-0.15, -0.15, _TABLE_HEIGHT + _LAPTOP_RESET_HEIGHT],
    high=[0.15, 0.15, _TABLE_HEIGHT + _LAPTOP_RESET_HEIGHT],
    single_sample=True,
)
...
class Laptop(aloha2_task.AlohaTask):
  """A task with a laptop on the tabletop."""

  def __init__(
      self,
      **kwargs,
  ):
    """Initializes a new `Laptop` task.
